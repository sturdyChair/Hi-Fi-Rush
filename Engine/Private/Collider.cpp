...
_bool CCollider::PointIntersect(XMVECTOR vPoint)
{
	return m_pBounding->PointIntersect(vPoint);
}
...
HRESULT CCollider::Render()
{
	if (/*GAMEINSTANCE->Get_DebugMode()*/true) {
		...
		m_pEffect->SetView(pGameInstance->Get_Transform_Matrix(CPipeLine::D3DTS_VIEW));
		m_pEffect->SetProjection(pGameInstance->Get_Transform_Matrix(CPipeLine::D3DTS_PROJ));
