...
#undef SET_ANIM_SPEED
    ...
    if (fileAddInfo.is_open())
    {
        ...
        _uint iEffectSize = 0;
        read_typed_data(fileAddInfo, iEffectSize);
        for (_uint i = 0; i < iEffectSize; ++i)
        {
            _float fFrame;
            read_typed_data(fileAddInfo, fFrame);
            Frame_Effects.emplace_back(fFrame);

            string strEffectName;
            size_t istringSize;
            read_typed_data(fileAddInfo, istringSize);
            strEffectName.resize(istringSize);
            fileAddInfo.read(&strEffectName[0], istringSize);
            EffectList.emplace_back(strEffectName);

            _float4x4 matTrans{};
            read_typed_data(fileAddInfo, matTrans);
            Effect_Trans_List.emplace_back(matTrans);

            _bool isFollow{};
            read_typed_data(fileAddInfo, isFollow);
            Effect_IsFollow_List.emplace_back(isFollow);

            _bool isBillBoard{};
            read_typed_data(fileAddInfo, isBillBoard);
            Effect_IsBillBoard_List.emplace_back(isBillBoard);
        }

    }
    fileAddInfo.close();

    // Load additional informations - Sound
    strFilePath = strFolderPath;
    strFilePath.append(szName);
    strFilePath.append(".snf");

    ifstream fileSoundInfo(strFilePath, ios::in | ios::binary);
    if (fileSoundInfo.is_open())
    {
        _uint iSize = 0;
        read_typed_data(fileSoundInfo, iSize);
        for (_uint i = 0; i < iSize; ++i)
        {
            _float fFrame;
            read_typed_data(fileSoundInfo, fFrame);
            Frame_Sounds.emplace_back(fFrame);

            wstring strSoundName;
            size_t istringSize;
            read_typed_data(fileSoundInfo, istringSize);
            strSoundName.resize(istringSize);
            fileSoundInfo.read(reinterpret_cast<char*>(&strSoundName[0]), istringSize * sizeof(_tchar));
            SoundList.emplace_back(strSoundName);

            _float fVolume;
            read_typed_data(fileSoundInfo, fVolume);
            Sound_Volume_List.emplace_back(fVolume);
        }
    }
    fileSoundInfo.close();
}
