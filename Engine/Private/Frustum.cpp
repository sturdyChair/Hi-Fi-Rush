...
void CFrustum::Update()
{
	...
	{
		vPoints[i] = XMVector3TransformCoord(XMLoadFloat3(&m_vPoints[i]), pGameInstance->Get_Transform_Matrix(CPipeLine::TS_PROJ_INV));
		vPoints[i] = XMVector3TransformCoord(vPoints[i], pGameInstance->Get_Transform_Matrix(CPipeLine::TS_VIEW_INV));

		XMStoreFloat3(&m_vWorldPoints[i], vPoints[i]);

		vPoints[i] = XMVector3TransformCoord(XMLoadFloat3(&m_vPoints[i]), XMMatrixInverse(nullptr, pGameInstance->Get_LightProj_Matrix()));
		vPoints[i] = XMVector3TransformCoord(vPoints[i], XMMatrixInverse(nullptr, pGameInstance->Get_LightView_Matrix()));

		XMStoreFloat3(&m_vWorldLightPoints[i], vPoints[i]);
	}

	/* 월드까지 변환해놓은 점 여덟개로 평면 여섯개를 만든다. */
	Make_Planes(m_vWorldPoints, m_vWorldPlanes);
	Make_Planes(m_vWorldLightPoints, m_vWorldLightPlanes);
}
...
_bool CFrustum::isIn_LightSpace(_fvector vWorldPos, _float fRange)
{
	for (size_t i = 0; i < 6; i++)
	{
		if (fRange < XMVectorGetX(XMPlaneDotCoord(XMLoadFloat4(&m_vWorldLightPlanes[i]), vWorldPos)))
			return false;
	}
	return true;
}

_bool CFrustum::isIn_WorldSpaceEX(_fvector vWorldPos, _float fRange)
{
