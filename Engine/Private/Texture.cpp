...
HRESULT CTexture::Initialize_Prototype(const _wstring& strTextureFilePath, _uint iNumTexture)
{
	...
	for (_uint i = 0; i < iNumTexture; ++i)
	{
		...
		if (lstrcmp(szExtension, TEXT(".dds")) == 0)
		{
			...
			{
				_tchar szPngFilePath[MAX_PATH] = TEXT("");
				wsprintf(szPngFilePath, L"%s%s%s%s", szDrive, szDir, szFileName, L".png");

				if (FAILED(CreateWICTextureFromFile(m_pDevice, szPngFilePath, nullptr, &m_SRVs[i])))
				{
					wstring errMsg = L"Failed to DDS and PNG load texture : \"";
					errMsg += strTextureFilePath;
					errMsg += L"\"";

					string str(errMsg.begin(), errMsg.end());
					cout << str << endl;
