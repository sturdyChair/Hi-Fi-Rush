...
_bool CMath_Manager::IsEqualWithEpsilon(_vector vec1, _vector vec2, float epsilon)
{
	return XMVector4NearEqual(vec1, vec2, XMVectorReplicate(epsilon));
}
...
_float4x4 CMath_Manager::Matrix_BillBoard(const _float4x4& _matWorld)
{
	_float4x4 matBillBoard = _matWorld;
	_matrix Mat = XMLoadFloat4x4(&matBillBoard);

	_float3 vScale = { XMVectorGetX(XMVector3Length(Mat.r[0])), XMVectorGetX(XMVector3Length(Mat.r[1])), XMVectorGetX(XMVector3Length(Mat.r[2])) };
	_vector vPos = { matBillBoard._41, matBillBoard._42, matBillBoard._43 };

	// 오브젝트에서 카메라로 향하는 방향을 반대로 설정
	_vector vLook = vPos - GAMEINSTANCE->Get_MainCamera()->Get_Transform()->Get_State(CTransform::STATE_POSITION);

	_vector vUp = XMVectorSet(0.f, 1.f, 0.f, 0.f);
	_vector vRight = XMVector3Cross(vUp, vLook);

	_float4 vTrueRight;
	XMStoreFloat4(&vTrueRight, XMVector3Normalize(vRight) * vScale.x);

	_float4 vTrueUp;
	XMStoreFloat4(&vTrueUp, vUp * vScale.y);

	_float4 vTrueLook;
	XMStoreFloat4(&vTrueLook, XMVector3Normalize(vLook) * vScale.z);

	memcpy(&matBillBoard._11, &vTrueRight.x, sizeof(vTrueRight));
	memcpy(&matBillBoard._21, &vTrueUp.x, sizeof(vTrueUp));
	memcpy(&matBillBoard._31, &vTrueLook.x, sizeof(vTrueLook));
