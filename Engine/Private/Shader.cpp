...
HRESULT CShader::Bind_ScalarIntValue(const _char* pConstantName, void* pData, _uint iSize, _uint iCount)
{
	ID3DX11EffectVariable* pVariable = m_pEffect->GetVariableByName(pConstantName);
	if (!pVariable)
		return E_FAIL;

	return pVariable->AsScalar()->SetIntArray((_int*)pData, 0, iCount);
}
