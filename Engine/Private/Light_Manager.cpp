...
shared_ptr<class CLight> CLight_Manager::Return_Light(string strLightTag)
{
	for (auto& pLight : m_Lights)
	{
		if (nullptr != pLight) {
...
void CLight_Manager::Remove_Light(string strLightTag)
{
	auto iter = m_Lights.begin();
	for (iter; iter != m_Lights.end();)
	{
		if (strLightTag == (*iter)->Get_LightDesc()->szName)
		{
...
HRESULT CLight_Manager::Bind_Lights(shared_ptr<class CShader> pShader)
{
	...
	_vector vCamPos = GAMEINSTANCE->Get_CamPosition_Vector();
	...
	for (auto& pLight : m_Lights)
	{
		if(pLight->Get_LightState())
		{
			...
			{
				_vector vDiff = XMLoadFloat4(&pDesc->vPosition) - vCamPos;
				_float fDist = XMVectorGetX(XMVector3Length(vDiff));
				if (fDist > m_fCullingRange)
				{
					continue;
				}
			}
...
void CLight_Manager::Rewrite_Description(string strLightTag, const LIGHT_DESC& Desc)
{
	for (auto& pLight : m_Lights)
	{
		if (nullptr != pLight) {
