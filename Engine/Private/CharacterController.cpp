...
void CCharacterController::Velocity_Calculation(_float fTimeDelta)
{
	...
	if (m_bGravity)
	{
		...
		{
			if (vVelocity.m128_f32[1] < -1.f)
			{
				vVelocity.m128_f32[1] = -1.f;
			}
		}
	}
...
PxExtendedVec3 CCharacterController::Get_FootPosition()
{
	return m_pPxController->getFootPosition();
}
...
void CCharacterController::Set_Gravity(_float fGravity)
{
	m_fGravity = fGravity;
}
