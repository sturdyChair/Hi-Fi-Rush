...
HRESULT CGameInstance::Initialize_Engine(_uint iNumLevels, const ENGINE_DESC& EngineDesc, _Out_ ID3D11Device** ppDevice, _Out_ ID3D11DeviceContext** ppContext)
{
	...
	m_iLoadingLevelIdx = EngineDesc.iLoadingLevelIdx;
	/* 인풋 디바이스 초기화. */
...
void CGameInstance::Clear(_uint iLevelIndex)
{
	...
	if(iLevelIndex != m_iLoadingLevelIdx)
		m_pObject_Pool->Clear_Pool();
	m_pComponent_Manager->Clear(iLevelIndex);
	m_pLightManager->LightClear();
...
void CGameInstance::Change_Level_Delayed(_uint iNextLevel)
{
	m_pLevel_Manager->Change_Level_Delayed(iNextLevel);
}
...
void CGameInstance::Set_ShadowDir(_fvector vDir)
{
	m_pPipeLine->Set_ShadowDir(vDir);
}

void CGameInstance::Set_ShadowDir(const _float4& vDir)
{
	m_pPipeLine->Set_ShadowDir(vDir);
}

_fmatrix CGameInstance::Get_LightView_Matrix() const
{
	return m_pPipeLine->Get_LightView_Matrix();
}

const _float4x4& CGameInstance::Get_LightView_Float4x4() const
{
	return m_pPipeLine->Get_LightView_Float4x4();
}

_fmatrix CGameInstance::Get_LightProj_Matrix() const
{
	return m_pPipeLine->Get_LightProj_Matrix();
}

const _float4x4& CGameInstance::Get_LightProj_Float4x4() const
{
	return m_pPipeLine->Get_LightProj_Float4x4();
}
...
shared_ptr<class CLight> CGameInstance::Return_Light(string strLightTag)
{
	return m_pLightManager->Return_Light(strLightTag);
}
void CGameInstance::Clear_Light()
{
	m_pLightManager->LightClear();
}
void CGameInstance::Remove_Light(string strLightTag)
{
	m_pLightManager->Remove_Light(strLightTag);
}
void CGameInstance::Rewrite_Description(string strLightTag, const LIGHT_DESC& Desc)
{
	m_pLightManager->Rewrite_Description(strLightTag, Desc);
}
_float* CGameInstance::Get_LightCullingPtr()
{
	return m_pLightManager->Get_LightCullingPtr();
}
...
shared_ptr<class CGameObject> CGameInstance::Pool_Object_Return(const wstring& strPrototypeTag, void* pArg)
{
	return m_pObject_Pool->Pool_Object_Return(strPrototypeTag, pArg);
}
...
void CGameInstance::Push_ObjectsPool(const wstring& strPrototypeTag, _uint iLevelIndex, const wstring& strLayerTag, void* pArg, _float4x4* TransformMat)
{
	m_pObject_Pool->Push_ObjectsPool(strPrototypeTag, iLevelIndex, strLayerTag, pArg, TransformMat);
}
...
void CGameInstance::Blur_On(const _float2& vCenter, _float fStrength)
{
...
void CGameInstance::Set_Fog(const _float4& vColor, _float fRange)
{
	m_pRenderer->Set_Fog(vColor, fRange);
}
...
bool CGameInstance::Is_Sound_Playing(SOUND_CHANNEL eChannel, const wstring& strSoundTag)
{
	return m_pSoundManager->Is_Sound_Playing(eChannel, strSoundTag);
}

bool CGameInstance::Is_Sound_3D_Playing(SOUND_CHANNEL eChannel, const wstring& strSoundTag)
{
	return m_pSoundManager->Is_Sound_3D_Playing(eChannel, strSoundTag);
}
...
void CGameInstance::Play_Unique_Sound_Channel(string ChannelName, SOUND_CHANNEL eChannel, const wstring& strSoundTag, bool bLoop)
{
	m_pSoundManager->Play_Unique_Sound_Channel(ChannelName, eChannel, strSoundTag, bLoop);
}

void CGameInstance::Add_Unique_Sound_Channel(string ChannelName)
{
	m_pSoundManager->Add_Unique_Sound_Channel(ChannelName);
}
...
void CGameInstance::Set_Sound_Position(SOUND_CHANNEL eChannel, const wstring& strSoundTag, _float fPosition)
{
	m_pSoundManager->Set_Sound_Position(eChannel, strSoundTag, fPosition);
}
...
void CGameInstance::Set_3DSound_Coord(SOUND_CHANNEL eChannel, const wstring& strSoundTag, _fvector vPos)
{
	m_pSoundManager->Set_3DSound_Coord(eChannel, strSoundTag, vPos);
}
...
_float4x4 CGameInstance::Matrix_BillBoard(const _float4x4& _matWorld)
{
	if (nullptr == m_pMath_Manager)
		assert(false);

	return m_pMath_Manager->Matrix_BillBoard(_matWorld);
}

_bool CGameInstance::IsEqualWithEpsilon(_vector vec1, _vector vec2, float epsilon)
{
	if (nullptr == m_pMath_Manager)
		assert(false);
	
	return m_pMath_Manager->IsEqualWithEpsilon(vec1, vec2, epsilon);
}
...
_bool CGameInstance::isIn_Frustum_WorldSpaceEX(_fvector vWorldPos, _float fRange)
{
	return m_pFrustum->isIn_WorldSpaceEX(vWorldPos, fRange);
}
...
_bool CGameInstance::isIn_Frustum_LightSpace(_fvector vWorldPos, _float fRange)
{
	return  m_pFrustum->isIn_LightSpace(vWorldPos, fRange);
}

#pragma endregion

#pragma region OCTREE
HRESULT CGameInstance::Initialize_Octree(_float fUnitScale, _int iMaxRecursion, const _float3& vCenter)
{
	return m_pOctree->Initialize(fUnitScale, iMaxRecursion, vCenter);
}
shared_ptr<COctree::Node> CGameInstance::Get_Node()
{
	return m_pOctree->Get_Node();
}
void CGameInstance::Clear_Octree()
{
	m_pOctree->Clear();
}
void CGameInstance::Start_Octree()
{
	m_pOctree->Start();
}
void CGameInstance::Stop_Octree()
{
	m_pOctree->Stop();
}
#pragma endregion