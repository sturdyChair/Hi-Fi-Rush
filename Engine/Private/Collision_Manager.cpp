...
void CCollision_Manager::Initialize()
{
	...
	CollisionAccessBoth(COLLISION_PLAYER, COLLISION_GIMMICK);
}
...
void CCollision_Manager::CollisionClear()
{
	...
	m_PrevColliderInfo = m_CurrColliderInfo;
	m_CurrColliderInfo.clear();
}
					...
					for (auto& pSrc : m_ListCollider[j])
					{
						...
						if (pSrc->Intersect(pDst))
						{
							if (iter->second)
							{
								...
								else if (!m_PrevColliderInfo.count(pDst->Get_ColliderID()) || !m_PrevColliderInfo.count(pSrc->Get_ColliderID()))
								{
...
void CCollision_Manager::Add_Collider(COLLIDER_TYPE _colliderType, shared_ptr<class CCollider> pCollider)
{
	...
	m_CurrColliderInfo.insert(pCollider->Get_ColliderID());
	pCollider->Set_Collider_Group(_colliderType);
}
