...
void CMesh::Cooking_TriangleMesh(const _float4x4& Transform)
{
	if(m_pTriangleMesh)
	{
	...
	vector<PxVec3> Vertex;
	for (auto& vert : vertices)
	{
		_vector Vec = XMVectorSet(vert.x, vert.y, vert.z, 1.f);
		Vec = XMVector3TransformCoord(Vec, Mat);
		PxVec3 pxVert;
		pxVert.x = Vec.m128_f32[0];
		pxVert.y = Vec.m128_f32[1];
		pxVert.z = Vec.m128_f32[2];
		Vertex.push_back(pxVert);
	}

	meshDesc.points.count = static_cast<PxU32>(Vertex.size());
	meshDesc.points.stride = sizeof(PxVec3);
	meshDesc.points.data = Vertex.data();
...
_bool CMesh::IsAffected(_uint iBoneIdx) const
{
	for (auto& idx : m_Bones)
	{
		if (idx == iBoneIdx)
			return true;
	}
...
HRESULT CMesh::Bind_Buffer_Instance(ID3D11Buffer* pInstanceBuffer)
{
	ID3D11Buffer* pVertexBuffers[] = {
	m_pVB,
	pInstanceBuffer
...
HRESULT CMesh::Draw_Instance(ID3D11Buffer* pInstanceBuffer)
{
	m_pContext->DrawIndexedInstanced(m_iNumIndexPerInstance, m_iNumInstance, 0, 0, 0);
...
void CMesh::Initialize_InstanceBuffer(_uint iNumMaxInstance)
{
	Safe_Release(m_pIB);
	m_iNumInstance = iNumMaxInstance;
...
void CMesh::Initialize_InstanceBuffer(_uint iNumMaxInstance, ID3D11Buffer** ppBufferOut)
{
	Safe_Release(m_pIB);
	m_iNumInstance = iNumMaxInstance;
	m_iInstanceVertexStride = sizeof(VTXMATRIX);
	...
	VTXMATRIX* pInstanceVertex = new VTXMATRIX[m_iNumInstance];
	ZeroMemory(pInstanceVertex, sizeof(VTXMATRIX) * m_iNumInstance);
	ZeroMemory(&m_BufferDesc, sizeof m_BufferDesc);
	...
	ZeroMemory(&m_InitialData, sizeof m_InitialData);
	m_InitialData.pSysMem = pInstanceVertex;
	if (FAILED(__super::Create_Buffer(ppBufferOut)))
	{
		assert(false);
	}
	m_iNumMaxInstance = m_iNumInstance;
	Safe_Delete_Array(pInstanceVertex);
}
#ifdef _DEBUG
