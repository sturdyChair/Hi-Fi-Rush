...
shared_ptr<class CGameObject> CObject_Pool::Pool_Object_Return(const wstring& strPrototypeTag, void* pArg)
{
    auto pClone = m_pGameInstance.lock()->Clone_Object(strPrototypeTag, pArg);
    if (!pClone)
        return nullptr;
    ...
    return m_mapPool[strPrototypeTag].back();
}
...
shared_ptr<CGameObject> CObject_Pool::Push_Object_From_Pool(const wstring& strPrototypeTag, _uint iLevelIndex, const wstring& strLayerTag, void* pArg, _float4x4* TransformMat)
{
    ...
    for (int i = 0; i < vecPool.size(); ++i)
    {
        if (vecPool[i]->Is_Dead())
        {
            ...
            pGameInstance->Push_Object(iLevelIndex, strLayerTag, vecPool[i]);
            return  vecPool[i];
        }
    }
...
void CObject_Pool::Push_ObjectsPool(const wstring& strPrototypeTag, _uint iLevelIndex, const wstring& strLayerTag, void* pArg, _float4x4* TransformMat)
{
    if (m_mapPool.count(strPrototypeTag) == 0)
    {
        return;
    ...
    vector<shared_ptr<CGameObject>>& vecPool = m_mapPool[strPrototypeTag];
    auto pGameInstance = m_pGameInstance.lock();
    for (auto pObject : vecPool)
    {
        pObject->Set_Dead(false);
        pObject->Push_From_Pool(pArg);
        if (TransformMat)
            pObject->Get_Transform()->Set_WorldMatrix(*TransformMat);
        pGameInstance->Push_Object(iLevelIndex, strLayerTag, pObject);
    }
}
