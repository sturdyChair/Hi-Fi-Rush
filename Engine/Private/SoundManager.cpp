...
HRESULT CSoundManager::Initialize(void)
{
    ...
    m_pFmodSystem->set3DSettings(1.f, 10.f, 1.f);
...
bool CSoundManager::Is_Sound_Playing(SOUND_CHANNEL eChannel, const wstring& strSoundTag)
{
    auto pSound = Find_Sound(eChannel, strSoundTag);
    if (pSound)
        return pSound->IsPlaying();
    return false;
}

bool CSoundManager::Is_Sound_3D_Playing(SOUND_CHANNEL eChannel, const wstring& strSoundTag)
{
    auto pSound = Find_Sound_3D(eChannel, strSoundTag);
    if (pSound)
        return pSound->IsPlaying();
    return false;
}
...
void CSoundManager::Play_Unique_Sound_Channel(string ChannelName ,SOUND_CHANNEL eChannel,  wstring strSoundTag, bool bLoop)
{
    auto iter = m_Unique_channel.find(ChannelName);

    if (iter != m_Unique_channel.end())
    {
        Pause_Sound(iter->second.first, iter->second.second);
        Play_Sound(eChannel, strSoundTag, bLoop);
        iter->second.first = eChannel;
        iter->second.second = strSoundTag;
    }
}

void CSoundManager::Add_Unique_Sound_Channel(string ChannelName)
{
    pair<SOUND_CHANNEL,  wstring> pairData = { SOUND_CHANNEL::CHANNEL_END , L"INIT"};
    m_Unique_channel.emplace(ChannelName, pairData);
}
...
void CSoundManager::Set_Sound_Position(SOUND_CHANNEL eChannel, const wstring& strSoundTag, _float fPosition)
{
    shared_ptr<CSound> pSound = Find_Sound(eChannel, strSoundTag);
    if (pSound)
    {
        pSound->SetPosition(fPosition * 100.f);
    }
}
...
void CSoundManager::Set_3DSound_Coord(SOUND_CHANNEL eChannel, const wstring& strSoundTag, _fvector vPos)
{
    auto pSound = Find_Sound_3D(eChannel, strSoundTag);
    if (pSound)
    {
        pSound->SetCoord(vPos);
    }
}
