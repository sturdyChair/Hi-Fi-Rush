...
void CVIBuffer_Instance::Range_Renewal(_float3 vRange)
{
	for (_uint i = 0; i < m_tDesc.iNumInstance; ++i)
	{
		...
		if (m_eStyle == STYLE_FOUNTAIN) {	
			float angleX = GAMEINSTANCE->Random_Float(0.0f, 2.0f * XM_PI);
			float angleY = GAMEINSTANCE->Random_Float(0.0f, XM_PI);

			float x = vRange.x * sin(angleY) * cos(angleX);
			float y = vRange.y * sin(angleY) * sin(angleX);
			float z = vRange.z * cos(angleY);
...
void CVIBuffer_Instance::Scale_Renewal(_float2 _vScale)
{
	...
	for (_uint i = 0; i < m_tDesc.iNumInstance; ++i)
	{
		...
		Transform = XMMatrixScaling(_vScale.x, _vScale.y, 1.f);
