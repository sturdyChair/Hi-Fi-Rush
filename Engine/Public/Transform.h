...
public:
	...
	void Set_Position(_float4 vVec) { Set_State(STATE_POSITION, XMLoadFloat4(&vVec)); }
	...
	void   Go_Vector(_fvector vDir, _float fTimeDelta);

	void	Rotation(_fvector vAxis, _float fAngle);
	void	Rotation_Quaternion(_fvector vRotation);
	void	Rotation_Override(_fvector vRotation);
	...
	void	LookAt_Vertical(_fvector vAt);
	void	LookAt_Vertical_Up(_fvector vAt);
