...
Texture2D gAlbedo, gNormal, gORM, gPosition, gEmission, gShadowMap;
Texture2D<uint> gId;
...
float4 PS_PBR(float4 pos : SV_POSITION, float2 texCoord : TEXCOORD) : SV_TARGET
{
    ...
    lighting *= 2.0f;
    float4 color = float4(lighting, albedo.a);
    float4 emssionColor = gEmission.Sample(PointSampler, texCoord);
    color.rgb += emssionColor.rgb * emssionColor.a;
...
{
    float4 vColor = (float4) 0;
    vColor = g_GlowTexture.Sample(LinearSampler, In.TexCoord);
    vColor.a = 0.1f;
    if (vColor.a < 0.01f)
        discard;
...
float4 PS_DISSOLVE(PS_IN input) : SV_TARGET0
{
    ...
    if (fNoise < g_Dissolve)
    {
        ...
        {
            vColor = lerp(vector(0.f, 0.f, 0.f, 1.0f), vColor, fDist / g_DissolveRange);
        }
        else
        {
            discard;
...
float4 PS_NPR(PS_IN input) : SV_TARGET0
{
    ...
    //float4 lightDepth = gShadowMap.Sample(PointSampler, input.TexCoord);

    float3 normal = normalize(normalTex.xyz * 2.0f - 1.0f);



    float3 viewDir = cameraPos.xyz - position.xyz;
    float fviewDist = length(viewDir);
    ...
    bool bAnim = normalTex.w > 0.1f;

    bool bNonLit = normalTex.w > 0.5f;
    if (bNonLit)
    {
        lighting = saturate(lighting * 2.f);
    }
    ...
    if(!bAnim)
    {
        float3 OutLine = (float3) 0;
        [unroll]
        for (uint n = 0; n < 9; n++)
            OutLine += LaplacianMask[n] * normalize((gNormal.Sample(PointSampler, input.TexCoord + float2(coord[n % 3] / 1280.f, coord[n / 3] / 720.f)).xyz * 2.0f - 1.f));
        float gray = 1.f - dot(grayScale, OutLine) / clamp(fviewDist / 10.f, 1.f, 100.f);
        if (gray < 0.3f)
            color.rgb = (float3) 0;
    }
    
    float fFogPower = saturate((fviewDist - g_fFogRange) / 500.f) * (g_fFogRange > 0.1f);
    
    color = color * (1.f - fFogPower) + g_vFogColor * fFogPower;
...
float4 PS_NPR_SHADOW(PS_IN input) : SV_TARGET0
{
    float4 albedo = gAlbedo.Sample(PointSampler, input.TexCoord);
    float4 normalTex = gNormal.Sample(PointSampler, input.TexCoord);
    float4 position = gPosition.Sample(PointSampler, input.TexCoord);
    ...
    //ao *= 0.25f;
    //lighting *= (1.f - ao);
    vector vShadowPos = mul(position, g_LightViewMatrix);
    vShadowPos = mul(vShadowPos, g_LightProjMatrix);
    float fShadowDepth = vShadowPos.w;
    
    vShadowPos /= vShadowPos.w;

    float2 vLightDepthTexcoord;

    vLightDepthTexcoord.x = vShadowPos.x * 0.5f + 0.5f;
    vLightDepthTexcoord.y = vShadowPos.y * -0.5f + 0.5f;
    
    vector vOldLightDepth = gShadowMap.Sample(PointSampler, vLightDepthTexcoord);
    ...
    float4 color = albedo * float4(lighting, 1.f);
    float4 emssionColor = gEmission.Sample(LinearSampler, input.TexCoord);
    
    if (fShadowDepth - 0.1f > fOldLightViewDepth && !bAnim)
        color.rgb *= 0.2f;
    color.rgb += emssionColor.rgb * emssionColor.a;
...
float4 PS_NPR_STENCIL(PS_IN input) : SV_TARGET0
{
    float4 albedo = gAlbedo.Sample(PointSampler, input.TexCoord);
    float4 normalTex = gNormal.Sample(PointSampler, input.TexCoord);
    float4 position = gPosition.Sample(PointSampler, input.TexCoord);
    //uint   iId = gId.Sample(PointSampler, input.TexCoord);
    int2 uv = int2(input.TexCoord.x / texScalerX, input.TexCoord.y / texScalerY);
    uint iId = gId.Load(int3(uv, 0));
    //float4 lightDepth = gShadowMap.Sample(PointSampler, input.TexCoord);
...
float4 PS_NPR_SHADOW_STENCIL(PS_IN input) : SV_TARGET0
{
    float4 albedo = gAlbedo.Sample(PointSampler, input.TexCoord);
    float4 normalTex = gNormal.Sample(PointSampler, input.TexCoord);
    float4 position = gPosition.Sample(PointSampler, input.TexCoord);
...
float4 PS_DEBUG(PS_IN input) : SV_TARGET0
{
    float4 vColor = gAlbedo.Sample(PointSampler, input.TexCoord);
...
technique11 DefaultTechnique
{
    ...
    pass NPR_SHADOW //16
    {
        ...
        HullShader = NULL;
        DomainShader = NULL;
        PixelShader = compile ps_5_0 PS_NPR_SHADOW();
    }
    pass Blur //17
    {
        SetDepthStencilState(DS_NonZ, 0);
    ...
    pass NPR_STENCIL //18
    {
        ...
        HullShader = NULL;
        DomainShader = NULL;
        PixelShader = compile ps_5_0 PS_NPR_STENCIL();
    }
    pass NPR_SHADOW_STENCIL //19
    {
        SetDepthStencilState(DS_NonZ, 0);
        SetRasterizerState(RS_Default);
        SetBlendState(BS_Default, float4(0.f, 0.f, 0.f, 1.f), 0xffffffff);
        ...
        HullShader = NULL;
        DomainShader = NULL;
        PixelShader = compile ps_5_0 PS_NPR_SHADOW_STENCIL();
    }
    pass Debug //20
    {
        SetDepthStencilState(DS_NonZ, 0);
