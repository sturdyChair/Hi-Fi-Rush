...
DepthStencilState DS_Outline_1
{
    DepthEnable = true;
    DepthWriteMask = all;
    StencilEnable = true;
...
DepthStencilState DS_Outline_2
{
    DepthEnable = true;
    DepthWriteMask = all;
    StencilEnable = true;
...
float4 normalMapSample(Texture2D normalMap, float2 TexCoord, float4 Tangent, float4 Bitangent, float4 Normal)
{
    ...
    float normalZ = sqrt(1.0f - dot(normalSample.xy, normalSample.xy));
    normalSample.b = normalZ;
    normalSample = normalize(normalSample);
    float3x3 TBN = float3x3(Tangent.xyz, Bitangent.xyz, Normal.xyz);
    float3 worldNormal = normalize(mul(normalSample, TBN));

    return float4(worldNormal * 0.5f + 0.5f, 0.0f) * -1.f; 
}
uint g_iObjectID;
