...
wstring CFont_Effect_Phaa::ObjID = TEXT("CFont_Effect_Phaa");

CFont_Effect_Phaa::CFont_Effect_Phaa(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CEffect_Base{ pDevice, pContext }
{
	s_PrototypeTags.insert(CFont_Effect_Phaa::ObjID);
}

CFont_Effect_Phaa::CFont_Effect_Phaa(const CFont_Effect_Phaa& rhs)
	: CEffect_Base(rhs)
{
}

CFont_Effect_Phaa::~CFont_Effect_Phaa()
{
	Free();
}

HRESULT CFont_Effect_Phaa::Initialize_Prototype()
{
	if (FAILED(__super::Initialize_Prototype()))
		return E_FAIL;

	m_List_ResourceInfo.push_back(make_shared<RES_TEXTURE>(TEXT("Prototype_Component_Effect_Phaa_Font"), TEXT("../Bin/Resources/Effect/Font/Korsica/T_VFX_tk_word_tc_PHAA_01.dds"), 1));
...
HRESULT CFont_Effect_Phaa::Initialize(void* pArg)
{
...
void CFont_Effect_Phaa::PriorityTick(_float fTimeDelta)
{


}

void CFont_Effect_Phaa::Tick(_float fTimeDelta)
{

	EffectTick(fTimeDelta);
}

void CFont_Effect_Phaa::LateTick(_float fTimeDelta)
{
	if (GetEffectIsView()) {
...
HRESULT CFont_Effect_Phaa::Render()
{
...
void CFont_Effect_Phaa::EffectStartCallBack()
{
}

void CFont_Effect_Phaa::EffectEndCallBack()
{

}

void CFont_Effect_Phaa::EffectSoftEnd()
{
}

void CFont_Effect_Phaa::EffectHardEnd()
{
}

HRESULT CFont_Effect_Phaa::Ready_Components()
{
	/* For.Com_Shader */
	...
	m_pTextureCom = static_pointer_cast<CTexture>(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Effect_Phaa_Font"), TEXT("Com_Texture")));
...
shared_ptr<CFont_Effect_Phaa> CFont_Effect_Phaa::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	struct MakeSharedEnabler :public CFont_Effect_Phaa
	{
		MakeSharedEnabler(ID3D11Device* pDevice, ID3D11DeviceContext* pContext) : CFont_Effect_Phaa(pDevice, pContext)
		{}
	};


	shared_ptr<CFont_Effect_Phaa> pInstance = make_shared<MakeSharedEnabler>(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : CFont_Effect_Phaa"));
		assert(false);
	}
...
shared_ptr<CGameObject> CFont_Effect_Phaa::Clone(void* pArg)
{
	struct MakeSharedEnabler :public CFont_Effect_Phaa
	{
		MakeSharedEnabler(const CFont_Effect_Phaa& rhs) : CFont_Effect_Phaa(rhs) {}
	};

	shared_ptr<CFont_Effect_Phaa> pInstance = make_shared<MakeSharedEnabler>(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : CFont_Effect_Phaa"));
		assert(false);
	}
