...
wstring CSquare_Flare_Effect::ObjID = TEXT("CSquare_Flare_Effect");

CSquare_Flare_Effect::CSquare_Flare_Effect(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CEffect_Base{ pDevice, pContext }
{
	s_PrototypeTags.insert(CSquare_Flare_Effect::ObjID);
}

CSquare_Flare_Effect::CSquare_Flare_Effect(const CSquare_Flare_Effect& rhs)
	: CEffect_Base(rhs)
{
}

CSquare_Flare_Effect::~CSquare_Flare_Effect()
{
	Free();
}

HRESULT CSquare_Flare_Effect::Initialize_Prototype()
{
	if (FAILED(__super::Initialize_Prototype()))
		return E_FAIL;

	m_List_ResourceInfo.push_back(make_shared<RES_TEXTURE>(TEXT("Prototype_Component_Effect_T_VFX_tk_scratch_22"), TEXT("../Bin/Resources/Effect/Flare/T_VFX_tk_scratch_22.png"), 1));
...
HRESULT CSquare_Flare_Effect::Initialize(void* pArg)
{
...
void CSquare_Flare_Effect::PriorityTick(_float fTimeDelta)
{
...
void CSquare_Flare_Effect::Tick(_float fTimeDelta)
{
	//SetEffectLoop(true);
...
void CSquare_Flare_Effect::LateTick(_float fTimeDelta)
{
	if (GetEffectIsView()) {
...
HRESULT CSquare_Flare_Effect::Render()
{
	_float4x4 ViewMat;
...
void CSquare_Flare_Effect::EffectStartCallBack()
{
}

void CSquare_Flare_Effect::EffectEndCallBack()
{

}

void CSquare_Flare_Effect::EffectSoftEnd()
{
}

void CSquare_Flare_Effect::EffectHardEnd()
{
}

HRESULT CSquare_Flare_Effect::Ready_Components()
{
	/* For.Com_Shader */
	...
	m_pTextureCom = static_pointer_cast<CTexture>(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Effect_T_VFX_tk_scratch_22"), TEXT("Com_Texture")));

	m_pNoiseTextureCom = static_pointer_cast<CTexture>(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Effect_T_VFX_tk_scratch_22"), TEXT("Com_Noise_Texture")));
...
shared_ptr<CSquare_Flare_Effect> CSquare_Flare_Effect::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	struct MakeSharedEnabler :public CSquare_Flare_Effect
	{
		MakeSharedEnabler(ID3D11Device* pDevice, ID3D11DeviceContext* pContext) : CSquare_Flare_Effect(pDevice, pContext)
		{}
	};


	shared_ptr<CSquare_Flare_Effect> pInstance = make_shared<MakeSharedEnabler>(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : CSquare_Flare_Effect"));
		assert(false);
	}
...
shared_ptr<CGameObject> CSquare_Flare_Effect::Clone(void* pArg)
{
	struct MakeSharedEnabler :public CSquare_Flare_Effect
	{
		MakeSharedEnabler(const CSquare_Flare_Effect& rhs) : CSquare_Flare_Effect(rhs) {}
	};

	shared_ptr<CSquare_Flare_Effect> pInstance = make_shared<MakeSharedEnabler>(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : CSquare_Flare_Effect"));
		assert(false);
	}
...
void CSquare_Flare_Effect::Free()
{
}