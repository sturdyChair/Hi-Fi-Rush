...
wstring CFont_Effect_Shoom::ObjID = TEXT("CFont_Effect_Shoom");

CFont_Effect_Shoom::CFont_Effect_Shoom(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CEffect_Base{ pDevice, pContext }
{
	s_PrototypeTags.insert(CFont_Effect_Shoom::ObjID);
}

CFont_Effect_Shoom::CFont_Effect_Shoom(const CFont_Effect_Shoom& rhs)
	: CEffect_Base(rhs)
{
}

CFont_Effect_Shoom::~CFont_Effect_Shoom()
{
	Free();
}

HRESULT CFont_Effect_Shoom::Initialize_Prototype()
{
	if (FAILED(__super::Initialize_Prototype()))
		return E_FAIL;

	m_List_ResourceInfo.push_back(make_shared<RES_TEXTURE>(TEXT("Prototype_Component_Effect_Shoom_Font"), TEXT("../Bin/Resources/Effect/Font/Korsica/T_VFX_tk_word_tc_SHOOM_01.dds"), 1));
...
HRESULT CFont_Effect_Shoom::Initialize(void* pArg)
{
...
void CFont_Effect_Shoom::PriorityTick(_float fTimeDelta)
{


}

void CFont_Effect_Shoom::Tick(_float fTimeDelta)
{

	EffectTick(fTimeDelta);
}

void CFont_Effect_Shoom::LateTick(_float fTimeDelta)
{
	if (GetEffectIsView()) {
...
HRESULT CFont_Effect_Shoom::Render()
{
...
void CFont_Effect_Shoom::EffectStartCallBack()
{
}

void CFont_Effect_Shoom::EffectEndCallBack()
{

}

void CFont_Effect_Shoom::EffectSoftEnd()
{
}

void CFont_Effect_Shoom::EffectHardEnd()
{
}

HRESULT CFont_Effect_Shoom::Ready_Components()
{
	/* For.Com_Shader */
	...
	m_pTextureCom = static_pointer_cast<CTexture>(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Effect_Shoom_Font"), TEXT("Com_Texture")));
...
shared_ptr<CFont_Effect_Shoom> CFont_Effect_Shoom::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	struct MakeSharedEnabler :public CFont_Effect_Shoom
	{
		MakeSharedEnabler(ID3D11Device* pDevice, ID3D11DeviceContext* pContext) : CFont_Effect_Shoom(pDevice, pContext)
		{}
	};


	shared_ptr<CFont_Effect_Shoom> pInstance = make_shared<MakeSharedEnabler>(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : CFont_Effect_Shoom"));
		assert(false);
	}
...
shared_ptr<CGameObject> CFont_Effect_Shoom::Clone(void* pArg)
{
	struct MakeSharedEnabler :public CFont_Effect_Shoom
	{
		MakeSharedEnabler(const CFont_Effect_Shoom& rhs) : CFont_Effect_Shoom(rhs) {}
	};

	shared_ptr<CFont_Effect_Shoom> pInstance = make_shared<MakeSharedEnabler>(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : CFont_Effect_Shoom"));
		assert(false);
	}
