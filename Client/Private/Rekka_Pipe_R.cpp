...
wstring CRekka_Pipe_R::ObjID = TEXT("Rekka_Pipe_R");

CRekka_Pipe_R::CRekka_Pipe_R(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
    : CPartObject{ pDevice, pContext }
{
}

CRekka_Pipe_R::CRekka_Pipe_R(const CRekka_Pipe_R& rhs)
    : CPartObject(rhs)
{
}

CRekka_Pipe_R::~CRekka_Pipe_R()
{
    Free();
}

HRESULT CRekka_Pipe_R::Initialize_Prototype()
{
    ...
    m_List_ResourceInfo.push_back(
        make_shared<RES_MODEL>(TEXT("Model_Rekka_Pipe_R"), TEXT("../Bin/Resources/Models/Monsters/Rekka/Pipe_L.fbx"), MODEL_TYPE::ANIM, preMatrix)
    );

    return S_OK;
}

HRESULT CRekka_Pipe_R::Initialize(void* pArg)
{
    ...
    // Default anim
    m_pModelCom->Set_AnimIndex("em2106_idle-pipe_000", true);
...
void CRekka_Pipe_R::PriorityTick(_float fTimeDelta)
{
}

void CRekka_Pipe_R::Tick(_float fTimeDelta)
{
    // Play Anim
    switch (m_curState)
    {
        ...
        {
            m_pModelCom->Set_AnimIndex("em2106_atk-pipe_300", true);
        ...
        default:
        {
            m_pModelCom->Set_AnimIndex("em2106_idle-pipe_000", false);
            m_isAttackOn = false;
            break;
        }
    }
...
void CRekka_Pipe_R::LateTick(_float fTimeDelta)
{
    if (m_isAttackOn)
...
void CRekka_Pipe_R::Collision_Enter(shared_ptr<CCollider> pOther, shared_ptr<CCollider> pCaller)
{
...
HRESULT CRekka_Pipe_R::Ready_Components()
{
    ...
    m_pModelCom = static_pointer_cast<CModel>(__super::Add_Component(m_iLevelIndex, TEXT("Model_Rekka_Pipe_R"), TEXT("Com_Model")));
...
void CRekka_Pipe_R::AttachOn()
{
    m_curState = EPipeState::Attach;
}

void CRekka_Pipe_R::PrepareAttack()
{
    m_curState = EPipeState::PreAttack;
}

void CRekka_Pipe_R::AttackSpecial()
{
...
shared_ptr<CRekka_Pipe_R> CRekka_Pipe_R::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
    struct MakeSharedEnabler :public CRekka_Pipe_R
    {
        MakeSharedEnabler(ID3D11Device* pDevice, ID3D11DeviceContext* pContext) : CRekka_Pipe_R(pDevice, pContext) {}
    };

    shared_ptr<CRekka_Pipe_R> pInstance = make_shared<MakeSharedEnabler>(pDevice, pContext);
    if (FAILED(pInstance->Initialize_Prototype()))
    {
        MSG_BOX(TEXT("Failed to Created : CRekka_Pipe_R"));
        assert(false);
    }
    return pInstance;
}

shared_ptr<CGameObject> CRekka_Pipe_R::Clone(void* pArg)
{
    struct MakeSharedEnabler :public CRekka_Pipe_R
    {
        MakeSharedEnabler(const CRekka_Pipe_R& rhs) : CRekka_Pipe_R(rhs) {}
    };

    shared_ptr<CRekka_Pipe_R> pInstance = make_shared<MakeSharedEnabler>(*this);
    if (FAILED(pInstance->Initialize(pArg)))
    {
        MSG_BOX(TEXT("Failed to Cloned : CRekka_Pipe_R"));
        assert(false);
    }
    return pInstance;
}

void CRekka_Pipe_R::Free()
{
}
