...
wstring C808_Flare_Effect::ObjID = TEXT("C808_Flare_Effect");

C808_Flare_Effect::C808_Flare_Effect(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CEffect_Base{ pDevice, pContext }
{
	s_PrototypeTags.insert(C808_Flare_Effect::ObjID);
}

C808_Flare_Effect::C808_Flare_Effect(const C808_Flare_Effect& rhs)
	: CEffect_Base(rhs)
{
}

C808_Flare_Effect::~C808_Flare_Effect()
{
	Free();
}

HRESULT C808_Flare_Effect::Initialize_Prototype()
{
	if (FAILED(__super::Initialize_Prototype()))
		return E_FAIL;

	m_List_ResourceInfo.push_back(make_shared<RES_TEXTURE>(TEXT("Prototype_Component_Effect_808Flare"), TEXT("../Bin/Resources/Effect/Flare/T_VFX_tk_ring_q_01.png"), 1));
...
HRESULT C808_Flare_Effect::Initialize(void* pArg)
{
...
void C808_Flare_Effect::PriorityTick(_float fTimeDelta)
{
...
void C808_Flare_Effect::Tick(_float fTimeDelta)
{
	//SetEffectLoop(true);
...
void C808_Flare_Effect::LateTick(_float fTimeDelta)
{
	if (GetEffectIsView()) {
...
HRESULT C808_Flare_Effect::Render()
{
	_float4x4 ViewMat;
...
void C808_Flare_Effect::EffectStartCallBack()
{
}

void C808_Flare_Effect::EffectEndCallBack()
{

}

void C808_Flare_Effect::EffectSoftEnd()
{
}

void C808_Flare_Effect::EffectHardEnd()
{
}

HRESULT C808_Flare_Effect::Ready_Components()
{
	/* For.Com_Shader */
	...
	m_pNoiseTextureCom = static_pointer_cast<CTexture>(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Effect_808Flare"), TEXT("Com_Noise_Texture")));
...
shared_ptr<C808_Flare_Effect> C808_Flare_Effect::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	struct MakeSharedEnabler :public C808_Flare_Effect
	{
		MakeSharedEnabler(ID3D11Device* pDevice, ID3D11DeviceContext* pContext) : C808_Flare_Effect(pDevice, pContext)
		{}
	};


	shared_ptr<C808_Flare_Effect> pInstance = make_shared<MakeSharedEnabler>(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : C808_Flare_Effect"));
		assert(false);
	}
...
shared_ptr<CGameObject> C808_Flare_Effect::Clone(void* pArg)
{
	struct MakeSharedEnabler :public C808_Flare_Effect
	{
		MakeSharedEnabler(const C808_Flare_Effect& rhs) : C808_Flare_Effect(rhs) {}
	};

	shared_ptr<C808_Flare_Effect> pInstance = make_shared<MakeSharedEnabler>(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : C808_Flare_Effect"));
		assert(false);
	}
