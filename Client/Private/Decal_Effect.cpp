...
wstring CDecal_Effect::ObjID = TEXT("CDecal_Effect");

CDecal_Effect::CDecal_Effect(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CEffect_Base{ pDevice, pContext }
{
	s_PrototypeTags.insert(CDecal_Effect::ObjID);
}

CDecal_Effect::CDecal_Effect(const CDecal_Effect& rhs)
	: CEffect_Base(rhs)
{
}

CDecal_Effect::~CDecal_Effect()
{
	Free();
}

HRESULT CDecal_Effect::Initialize_Prototype()
{
...
HRESULT CDecal_Effect::Initialize(void* pArg)
{
...
void CDecal_Effect::PriorityTick(_float fTimeDelta)
{
}

void CDecal_Effect::Tick(_float fTimeDelta)
{
	EffectTick(fTimeDelta);
}

void CDecal_Effect::LateTick(_float fTimeDelta)
{
	if (GetEffectIsView()) {
...
void CDecal_Effect::EffectStartCallBack()
{

}

void CDecal_Effect::EffectEndCallBack()
{

}

void CDecal_Effect::EffectSoftEnd()
{

}

void CDecal_Effect::EffectHardEnd()
{

}

HRESULT CDecal_Effect::Ready_Components()
{
	/* For.Com_Shader */
...
shared_ptr<CDecal_Effect> CDecal_Effect::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	struct MakeSharedEnabler :public CDecal_Effect
	{
		MakeSharedEnabler(ID3D11Device* pDevice, ID3D11DeviceContext* pContext) : CDecal_Effect(pDevice, pContext)
		{}
	};


	shared_ptr<CDecal_Effect> pInstance = make_shared<MakeSharedEnabler>(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : CDecal_Effect"));
		assert(false);
	}
...
shared_ptr<CGameObject> CDecal_Effect::Clone(void* pArg)
{
	struct MakeSharedEnabler :public CDecal_Effect
	{
		MakeSharedEnabler(const CDecal_Effect& rhs) : CDecal_Effect(rhs) {}
	};

	shared_ptr<CDecal_Effect> pInstance = make_shared<MakeSharedEnabler>(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : CDecal_Effect"));
		assert(false);
	}
