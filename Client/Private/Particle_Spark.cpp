...
wstring CParticle_Spark::ObjID = TEXT("CParticle_Spark");

CParticle_Spark::CParticle_Spark(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CEffect_Base{ pDevice, pContext }
{
	s_PrototypeTags.insert(CParticle_Spark::ObjID);
}
...
CParticle_Spark::~CParticle_Spark()
{
	Free();
}
...
HRESULT CParticle_Spark::Initialize_Prototype()
{
	...
	InstanceDesc.vTorque = _float3(0.f,0.f,0.f);
	InstanceDesc.bLoop = true;


	if (FAILED(GAMEINSTANCE->Add_Prototype(LEVEL_STATIC, TEXT("Prototype_Component_VIBuffer_PointInstance_Particle_Spark_Base"),
...
void CParticle_Spark::PriorityTick(_float fTimeDelta)
{
	PARENTCHECK
}
...
void CParticle_Spark::EffectStartCallBack()
{
}

void CParticle_Spark::EffectEndCallBack()
{
...
void CParticle_Spark::EffectSoftEnd()
{
}

void CParticle_Spark::EffectHardEnd()
{
}

_float3 CParticle_Spark::Get_Pivot()
{
	if (m_pVIBufferCom == nullptr)
...
void CParticle_Spark::Set_Pivot(_float3 vPivot)
{
	if (m_pVIBufferCom == nullptr)
		return;
...
_float2 CParticle_Spark::Get_Speed()
{
	if (m_pVIBufferCom == nullptr)
...
void CParticle_Spark::Set_Speed(_float2 vSpeed)
{
	if (m_pVIBufferCom == nullptr)
		return;
...
_float3 CParticle_Spark::Get_Range()
{
	if (m_pVIBufferCom == nullptr)
		return _float3(0.f, 0.f, 0.f);
...
void CParticle_Spark::Set_Range(_float3 vRange)
{
	if (m_pVIBufferCom == nullptr)
		return;
...
_bool CParticle_Spark::Get_IsRandomize()
{
	if (m_pVIBufferCom == nullptr)
		return false;
...
void CParticle_Spark::Set_IsRandomize(_bool bIsRandomize)
{
	if (m_pVIBufferCom == nullptr)
		return;
...
_float2 CParticle_Spark::Get_Scale()
{
	if (m_pVIBufferCom == nullptr)
		return _float2(0.f, 0.f);
...
void CParticle_Spark::Set_Scale(_float2 vScale)
{
	if (m_pVIBufferCom == nullptr)
		return;
...
CVIBuffer_Instance::INSTANCE_STYLE CParticle_Spark::Get_Style()
{
	if (m_pVIBufferCom == nullptr)
		assert(false);
...
void CParticle_Spark::Set_Style(CVIBuffer_Instance::INSTANCE_STYLE eStyle)
{
	if (m_pVIBufferCom == nullptr)
		return;
...
HRESULT CParticle_Spark::Ready_Components()
{
	...
	m_pVIBufferCom = static_pointer_cast<CVIBuffer_Point_Instance>(Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_VIBuffer_PointInstance_Particle_Spark_Base"), TEXT("Com_VIBuffer")));
...
shared_ptr<CParticle_Spark> CParticle_Spark::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	struct MakeSharedEnabler :public CParticle_Spark
	{
		MakeSharedEnabler(ID3D11Device* pDevice, ID3D11DeviceContext* pContext) : CParticle_Spark(pDevice, pContext)
		{}
	};


	shared_ptr<CParticle_Spark> pInstance = make_shared<MakeSharedEnabler>(pDevice, pContext);
...
shared_ptr<CGameObject> CParticle_Spark::Clone(void* pArg)
{
	struct MakeSharedEnabler :public CParticle_Spark
	{
		MakeSharedEnabler(const CParticle_Spark& rhs) : CParticle_Spark(rhs) {}
	};

	shared_ptr<CParticle_Spark> pInstance = make_shared<MakeSharedEnabler>(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : CParticle_Spark"));
		assert(false);
	}
