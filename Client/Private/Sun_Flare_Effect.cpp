...
wstring CSun_Flare_Effect::ObjID = TEXT("CSun_Flare_Effect");

CSun_Flare_Effect::CSun_Flare_Effect(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CEffect_Base{ pDevice, pContext }
{
	s_PrototypeTags.insert(CSun_Flare_Effect::ObjID);
}

CSun_Flare_Effect::CSun_Flare_Effect(const CSun_Flare_Effect& rhs)
	: CEffect_Base(rhs)
{
}

CSun_Flare_Effect::~CSun_Flare_Effect()
{
	Free();
}

HRESULT CSun_Flare_Effect::Initialize_Prototype()
{
...
HRESULT CSun_Flare_Effect::Initialize(void* pArg)
{
...
void CSun_Flare_Effect::PriorityTick(_float fTimeDelta)
{
...
void CSun_Flare_Effect::Tick(_float fTimeDelta)
{
	//SetEffectLoop(true);
...
void CSun_Flare_Effect::LateTick(_float fTimeDelta)
{
	if (GetEffectIsView()) {
...
HRESULT CSun_Flare_Effect::Render()
{
	_float4x4 ViewMat;
...
void CSun_Flare_Effect::EffectStartCallBack()
{
}

void CSun_Flare_Effect::EffectEndCallBack()
{

}

void CSun_Flare_Effect::EffectSoftEnd()
{
}

void CSun_Flare_Effect::EffectHardEnd()
{
}

HRESULT CSun_Flare_Effect::Ready_Components()
{
	/* For.Com_Shader */
	...
	m_pTextureCom = static_pointer_cast<CTexture>(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Effect_T_VFX_tk_flare_03"), TEXT("Com_Texture")));

	m_pNoiseTextureCom = static_pointer_cast<CTexture>(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Effect_T_VFX_tk_flare_03"), TEXT("Com_Noise_Texture")));
...
shared_ptr<CSun_Flare_Effect> CSun_Flare_Effect::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	struct MakeSharedEnabler :public CSun_Flare_Effect
	{
		MakeSharedEnabler(ID3D11Device* pDevice, ID3D11DeviceContext* pContext) : CSun_Flare_Effect(pDevice, pContext)
		{}
	};


	shared_ptr<CSun_Flare_Effect> pInstance = make_shared<MakeSharedEnabler>(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : CSun_Flare_Effect"));
		assert(false);
	}
...
shared_ptr<CGameObject> CSun_Flare_Effect::Clone(void* pArg)
{
	struct MakeSharedEnabler :public CSun_Flare_Effect
	{
		MakeSharedEnabler(const CSun_Flare_Effect& rhs) : CSun_Flare_Effect(rhs) {}
	};

	shared_ptr<CSun_Flare_Effect> pInstance = make_shared<MakeSharedEnabler>(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : CSun_Flare_Effect"));
		assert(false);
	}
