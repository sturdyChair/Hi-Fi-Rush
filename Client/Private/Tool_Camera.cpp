...
void CCamera_Tool::PriorityTick(_float fTimeDelta)
{
	...
	{
		m_pTransformCom->Set_Position(m_pTransformCom->Get_Position() + vUp * fTimeDelta * 10.f);
	}
	...
	;
	//m_pTransformCom->Set_Speed(m_pTransformCom->Get_Speed() + m_pGameInstance.lock()->Get_DIMouseMove(MOUSEMOVESTATE::DIMM_WHEEL));
