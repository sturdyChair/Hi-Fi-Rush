...
wstring CSpectra_Mark::ObjID = TEXT("CSpectra_Mark");

CSpectra_Mark::CSpectra_Mark(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CEffect_Base{ pDevice, pContext }
{
	s_PrototypeTags.insert(CSpectra_Mark::ObjID);
}

CSpectra_Mark::CSpectra_Mark(const CSpectra_Mark& rhs)
	: CEffect_Base(rhs)
{
}

CSpectra_Mark::~CSpectra_Mark()
{
	Free();
}

HRESULT CSpectra_Mark::Initialize_Prototype()
{
	...
	m_List_ResourceInfo.push_back(make_shared<RES_TEXTURE>(TEXT("Prototype_Component_Effect_Spectra"), TEXT("../Bin/Resources/Effect/Flare/Spectra.png"), 1));

	return S_OK;
}

HRESULT CSpectra_Mark::Initialize(void* pArg)
{
...
void CSpectra_Mark::PriorityTick(_float fTimeDelta)
{
...
void CSpectra_Mark::Tick(_float fTimeDelta)
{
	//SetEffectLoop(true);
...
void CSpectra_Mark::LateTick(_float fTimeDelta)
{
	if (GetEffectIsView()) {
...
void CSpectra_Mark::EffectStartCallBack()
{
}

void CSpectra_Mark::EffectEndCallBack()
{

}

void CSpectra_Mark::EffectSoftEnd()
{
}

void CSpectra_Mark::EffectHardEnd()
{
}

HRESULT CSpectra_Mark::Ready_Components()
{
	/* For.Com_Shader */
	...
	m_pTextureCom = static_pointer_cast<CTexture>(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Effect_Spectra"), TEXT("Com_Texture")));
...
shared_ptr<CSpectra_Mark> CSpectra_Mark::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	struct MakeSharedEnabler :public CSpectra_Mark
	{
		MakeSharedEnabler(ID3D11Device* pDevice, ID3D11DeviceContext* pContext) : CSpectra_Mark(pDevice, pContext)
		{}
	};


	shared_ptr<CSpectra_Mark> pInstance = make_shared<MakeSharedEnabler>(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : CSpectra_Mark"));
		assert(false);
	}
...
shared_ptr<CGameObject> CSpectra_Mark::Clone(void* pArg)
{
	struct MakeSharedEnabler :public CSpectra_Mark
	{
		MakeSharedEnabler(const CSpectra_Mark& rhs) : CSpectra_Mark(rhs) {}
	};

	shared_ptr<CSpectra_Mark> pInstance = make_shared<MakeSharedEnabler>(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : CSpectra_Mark"));
		assert(false);
	}
