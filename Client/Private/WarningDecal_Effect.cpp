...
wstring CWarningDecal_Effect::ObjID = TEXT("CWarningDecal_Effect");

CWarningDecal_Effect::CWarningDecal_Effect(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CEffect_Base{ pDevice, pContext }
{
	s_PrototypeTags.insert(CWarningDecal_Effect::ObjID);
}

CWarningDecal_Effect::CWarningDecal_Effect(const CWarningDecal_Effect& rhs)
	: CEffect_Base(rhs)
{
}

CWarningDecal_Effect::~CWarningDecal_Effect()
{
	Free();
}

HRESULT CWarningDecal_Effect::Initialize_Prototype()
{
...
HRESULT CWarningDecal_Effect::Initialize(void* pArg)
{
...
void CWarningDecal_Effect::PriorityTick(_float fTimeDelta)
{
}

void CWarningDecal_Effect::Tick(_float fTimeDelta)
{
...
void CWarningDecal_Effect::LateTick(_float fTimeDelta)
{
	if (GetEffectIsView()) {
...
HRESULT CWarningDecal_Effect::Render()
{
	_float4x4 ViewMat;
	...
	if (FAILED(m_pShaderCom->Begin((_uint)CUBE_PASS::SSD_WARNING)))
		return E_FAIL;
...
void CWarningDecal_Effect::EffectStartCallBack()
{
	m_fThreshold = 0.f;
}

void CWarningDecal_Effect::EffectEndCallBack()
{

}

void CWarningDecal_Effect::EffectSoftEnd()
{

}

void CWarningDecal_Effect::EffectHardEnd()
{

}

HRESULT CWarningDecal_Effect::Ready_Components()
{
	/* For.Com_Shader */
...
shared_ptr<CWarningDecal_Effect> CWarningDecal_Effect::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	struct MakeSharedEnabler :public CWarningDecal_Effect
	{
		MakeSharedEnabler(ID3D11Device* pDevice, ID3D11DeviceContext* pContext) : CWarningDecal_Effect(pDevice, pContext)
		{}
	};


	shared_ptr<CWarningDecal_Effect> pInstance = make_shared<MakeSharedEnabler>(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : CWarningDecal_Effect"));
		assert(false);
	}
...
shared_ptr<CGameObject> CWarningDecal_Effect::Clone(void* pArg)
{
	struct MakeSharedEnabler :public CWarningDecal_Effect
	{
		MakeSharedEnabler(const CWarningDecal_Effect& rhs) : CWarningDecal_Effect(rhs) {}
	};

	shared_ptr<CWarningDecal_Effect> pInstance = make_shared<MakeSharedEnabler>(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : CWarningDecal_Effect"));
		assert(false);
	}
...
void CWarningDecal_Effect::Free()
{
}