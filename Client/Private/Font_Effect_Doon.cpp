...
wstring CFont_Effect_Doon::ObjID = TEXT("CFont_Effect_Doon");

CFont_Effect_Doon::CFont_Effect_Doon(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CEffect_Base{ pDevice, pContext }
{
	s_PrototypeTags.insert(CFont_Effect_Doon::ObjID);
}

CFont_Effect_Doon::CFont_Effect_Doon(const CFont_Effect_Doon& rhs)
	: CEffect_Base(rhs)
{
}

CFont_Effect_Doon::~CFont_Effect_Doon()
{
	Free();
}

HRESULT CFont_Effect_Doon::Initialize_Prototype()
{
...
HRESULT CFont_Effect_Doon::Initialize(void* pArg)
{
...
void CFont_Effect_Doon::PriorityTick(_float fTimeDelta)
{


}

void CFont_Effect_Doon::Tick(_float fTimeDelta)
{

	EffectTick(fTimeDelta);
}

void CFont_Effect_Doon::LateTick(_float fTimeDelta)
{
	if (GetEffectIsView()) {
...
HRESULT CFont_Effect_Doon::Render()
{
...
void CFont_Effect_Doon::EffectStartCallBack()
{
}

void CFont_Effect_Doon::EffectEndCallBack()
{

}

void CFont_Effect_Doon::EffectSoftEnd()
{
}

void CFont_Effect_Doon::EffectHardEnd()
{
}

HRESULT CFont_Effect_Doon::Ready_Components()
{
	/* For.Com_Shader */
...
shared_ptr<CFont_Effect_Doon> CFont_Effect_Doon::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	struct MakeSharedEnabler :public CFont_Effect_Doon
	{
		MakeSharedEnabler(ID3D11Device* pDevice, ID3D11DeviceContext* pContext) : CFont_Effect_Doon(pDevice, pContext)
		{}
	};


	shared_ptr<CFont_Effect_Doon> pInstance = make_shared<MakeSharedEnabler>(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : CFont_Effect_Doon"));
		assert(false);
	}
...
shared_ptr<CGameObject> CFont_Effect_Doon::Clone(void* pArg)
{
	struct MakeSharedEnabler :public CFont_Effect_Doon
	{
		MakeSharedEnabler(const CFont_Effect_Doon& rhs) : CFont_Effect_Doon(rhs) {}
	};

	shared_ptr<CFont_Effect_Doon> pInstance = make_shared<MakeSharedEnabler>(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : CFont_Effect_Doon"));
		assert(false);
	}
