...
wstring CDecal_Ring_Effect::ObjID = TEXT("CDecal_Ring_Effect");

CDecal_Ring_Effect::CDecal_Ring_Effect(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CEffect_Base{ pDevice, pContext }
{
	s_PrototypeTags.insert(CDecal_Ring_Effect::ObjID);
}

CDecal_Ring_Effect::CDecal_Ring_Effect(const CDecal_Ring_Effect& rhs)
	: CEffect_Base(rhs)
{
}

CDecal_Ring_Effect::~CDecal_Ring_Effect()
{
	Free();
}

HRESULT CDecal_Ring_Effect::Initialize_Prototype()
{
...
HRESULT CDecal_Ring_Effect::Initialize(void* pArg)
{
...
void CDecal_Ring_Effect::PriorityTick(_float fTimeDelta)
{
}

void CDecal_Ring_Effect::Tick(_float fTimeDelta)
{
	EffectTick(fTimeDelta);
}

void CDecal_Ring_Effect::LateTick(_float fTimeDelta)
{
	if (GetEffectIsView()) {
...
HRESULT CDecal_Ring_Effect::Render()
{
	_float4x4 ViewMat;
...
void CDecal_Ring_Effect::EffectStartCallBack()
{

}

void CDecal_Ring_Effect::EffectEndCallBack()
{

}

void CDecal_Ring_Effect::EffectSoftEnd()
{

}

void CDecal_Ring_Effect::EffectHardEnd()
{

}

HRESULT CDecal_Ring_Effect::Ready_Components()
{
	/* For.Com_Shader */
...
shared_ptr<CDecal_Ring_Effect> CDecal_Ring_Effect::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	struct MakeSharedEnabler :public CDecal_Ring_Effect
	{
		MakeSharedEnabler(ID3D11Device* pDevice, ID3D11DeviceContext* pContext) : CDecal_Ring_Effect(pDevice, pContext)
		{}
	};


	shared_ptr<CDecal_Ring_Effect> pInstance = make_shared<MakeSharedEnabler>(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : CDecal_Ring_Effect"));
		assert(false);
	}
...
shared_ptr<CGameObject> CDecal_Ring_Effect::Clone(void* pArg)
{
	struct MakeSharedEnabler :public CDecal_Ring_Effect
	{
		MakeSharedEnabler(const CDecal_Ring_Effect& rhs) : CDecal_Ring_Effect(rhs) {}
	};

	shared_ptr<CDecal_Ring_Effect> pInstance = make_shared<MakeSharedEnabler>(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : CDecal_Ring_Effect"));
		assert(false);
	}
