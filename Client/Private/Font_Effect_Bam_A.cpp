...
wstring CFont_Effect_Bam_A::ObjID = TEXT("CFont_Effect_Bam_A");

CFont_Effect_Bam_A::CFont_Effect_Bam_A(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CEffect_Base{ pDevice, pContext }
{
	s_PrototypeTags.insert(CFont_Effect_Bam_A::ObjID);
}

CFont_Effect_Bam_A::CFont_Effect_Bam_A(const CFont_Effect_Bam_A& rhs)
	: CEffect_Base(rhs)
{
}

CFont_Effect_Bam_A::~CFont_Effect_Bam_A()
{
	Free();
}

HRESULT CFont_Effect_Bam_A::Initialize_Prototype()
{
...
HRESULT CFont_Effect_Bam_A::Initialize(void* pArg)
{
...
void CFont_Effect_Bam_A::PriorityTick(_float fTimeDelta)
{


}

void CFont_Effect_Bam_A::Tick(_float fTimeDelta)
{

	EffectTick(fTimeDelta);
}

void CFont_Effect_Bam_A::LateTick(_float fTimeDelta)
{
	if (GetEffectIsView()) {
...
HRESULT CFont_Effect_Bam_A::Render()
{
...
void CFont_Effect_Bam_A::EffectStartCallBack()
{
}

void CFont_Effect_Bam_A::EffectEndCallBack()
{

}

void CFont_Effect_Bam_A::EffectSoftEnd()
{
}

void CFont_Effect_Bam_A::EffectHardEnd()
{
}

HRESULT CFont_Effect_Bam_A::Ready_Components()
{
	/* For.Com_Shader */
...
shared_ptr<CFont_Effect_Bam_A> CFont_Effect_Bam_A::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	struct MakeSharedEnabler :public CFont_Effect_Bam_A
	{
		MakeSharedEnabler(ID3D11Device* pDevice, ID3D11DeviceContext* pContext) : CFont_Effect_Bam_A(pDevice, pContext)
		{}
	};


	shared_ptr<CFont_Effect_Bam_A> pInstance = make_shared<MakeSharedEnabler>(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : CFont_Effect_Bam_A"));
		assert(false);
	}
...
shared_ptr<CGameObject> CFont_Effect_Bam_A::Clone(void* pArg)
{
	struct MakeSharedEnabler :public CFont_Effect_Bam_A
	{
		MakeSharedEnabler(const CFont_Effect_Bam_A& rhs) : CFont_Effect_Bam_A(rhs) {}
	};

	shared_ptr<CFont_Effect_Bam_A> pInstance = make_shared<MakeSharedEnabler>(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : CFont_Effect_Bam_A"));
		assert(false);
	}
