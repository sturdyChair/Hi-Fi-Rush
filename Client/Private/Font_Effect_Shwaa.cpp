...
wstring CFont_Effect_Shwaa::ObjID = TEXT("CFont_Effect_Shwaa");

CFont_Effect_Shwaa::CFont_Effect_Shwaa(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CEffect_Base{ pDevice, pContext }
{
	s_PrototypeTags.insert(CFont_Effect_Shwaa::ObjID);
}

CFont_Effect_Shwaa::CFont_Effect_Shwaa(const CFont_Effect_Shwaa& rhs)
	: CEffect_Base(rhs)
{
}

CFont_Effect_Shwaa::~CFont_Effect_Shwaa()
{
	Free();
}

HRESULT CFont_Effect_Shwaa::Initialize_Prototype()
{
	if (FAILED(__super::Initialize_Prototype()))
		return E_FAIL;

	m_List_ResourceInfo.push_back(make_shared<RES_TEXTURE>(TEXT("Prototype_Component_Effect_Shwaa_Font"), TEXT("../Bin/Resources/Effect/Font/Korsica/T_VFX_tk_word_tc_SHWAA_01.dds"), 1));
...
HRESULT CFont_Effect_Shwaa::Initialize(void* pArg)
{
...
void CFont_Effect_Shwaa::PriorityTick(_float fTimeDelta)
{


}

void CFont_Effect_Shwaa::Tick(_float fTimeDelta)
{

	EffectTick(fTimeDelta);
}

void CFont_Effect_Shwaa::LateTick(_float fTimeDelta)
{
	if (GetEffectIsView()) {
...
HRESULT CFont_Effect_Shwaa::Render()
{
...
void CFont_Effect_Shwaa::EffectStartCallBack()
{
}

void CFont_Effect_Shwaa::EffectEndCallBack()
{

}

void CFont_Effect_Shwaa::EffectSoftEnd()
{
}

void CFont_Effect_Shwaa::EffectHardEnd()
{
}

HRESULT CFont_Effect_Shwaa::Ready_Components()
{
	/* For.Com_Shader */
	...
	m_pTextureCom = static_pointer_cast<CTexture>(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Effect_Shwaa_Font"), TEXT("Com_Texture")));
...
shared_ptr<CFont_Effect_Shwaa> CFont_Effect_Shwaa::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	struct MakeSharedEnabler :public CFont_Effect_Shwaa
	{
		MakeSharedEnabler(ID3D11Device* pDevice, ID3D11DeviceContext* pContext) : CFont_Effect_Shwaa(pDevice, pContext)
		{}
	};


	shared_ptr<CFont_Effect_Shwaa> pInstance = make_shared<MakeSharedEnabler>(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : CFont_Effect_Shwaa"));
		assert(false);
	}
...
shared_ptr<CGameObject> CFont_Effect_Shwaa::Clone(void* pArg)
{
	struct MakeSharedEnabler :public CFont_Effect_Shwaa
	{
		MakeSharedEnabler(const CFont_Effect_Shwaa& rhs) : CFont_Effect_Shwaa(rhs) {}
	};

	shared_ptr<CFont_Effect_Shwaa> pInstance = make_shared<MakeSharedEnabler>(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : CFont_Effect_Shwaa"));
		assert(false);
	}
