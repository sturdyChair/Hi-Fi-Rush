...
CUi_UpRightBack::CUi_UpRightBack(const CUi_UpRightBack& rhs)
	:CUi_Default(rhs)
{
}
...
void CUi_UpRightBack::Tick(_float fTimeDelta)
{
	...
	_uint Chip = PLAYERMANAGER->Get_PlayerPauseChip();
	m_FontChip.String = to_wstring(Chip);
}
...
void CUi_UpRightBack::LateTick(_float fTimeDelta)
{
	if (m_bActive)
	{
		...
		m_FontChip.DirectPos = { 585 - m_FontDesc.String.size() * 7.5f - 80, 330 };
		m_pChip->Set_Pos(585 - m_FontDesc.String.size() * 7.5f - 100, m_pChip->Get_Pos().y);
...
void CUi_UpRightBack::Set_Color(_float4 Color)
{
	...
	m_FontChip.Color = Color;
	m_pGear->Set_Color(Color);
	m_pChip->Set_Color(Color);
}
...
void CUi_UpRightBack::Set_Color(ColorType Type, _float Color)
{
	...
	case ColorType::R:
		...
		m_FontChip.Color.x = Color;
		break;
	case ColorType::G:
		m_fColor.y = Color;
		m_FontDesc.Color.y = Color;
		m_FontChip.Color.y = Color;
		break;
	case ColorType::B:
		m_fColor.z = Color;
		m_FontDesc.Color.z = Color;
		m_FontChip.Color.z = Color;
		break;
	case ColorType::A:
		m_fColor.w = Color;
		m_FontDesc.Color.w = Color;
		m_FontChip.Color.w = Color;
		break;
	default:
		assert(false);
		break;
...
void CUi_UpRightBack::Add_Color(_float4 Color)
{
	...
	m_FontChip.Color.x += Color.x;
	m_FontChip.Color.y += Color.y;
	m_FontChip.Color.z += Color.z;
	m_FontChip.Color.w += Color.w;

	m_pGear->Add_Color(Color);
	m_pChip->Add_Color(Color);
}
...
void CUi_UpRightBack::Set_Active(_bool Active)
{
	...
	m_pChip->Set_Active(Active);
}

void CUi_UpRightBack::Dying()
{
	m_pGear->Dead();
	m_pChip->Dead();
}
...
void CUi_UpRightBack::Initialize_Gear()
{
	...
	m_pGear = static_pointer_cast<CUi_Default>(CUi_Mgr::Get_Instance()
		->Add_Clone_Return(CUi_Default::ObjID, m_iLevelIndex, L"CUi", CUi_Default::ObjID, &Desc));
}

void CUi_UpRightBack::Initialize_Chip()
{
	CUi_Default::Desc Desc;
	Desc.fSizeX = 40;
	Desc.fSizeY = 40;
	Desc.fX = 585 - m_FontDesc.String.size() * 15.f - 120;
	Desc.fY = 325;
	Desc.fZ = 0.6f;
	Desc.eRenderType = m_eRenderType;
	Desc.iPriority = m_iRenderPriority + 1;
	...
	Desc.TextureLevel = LEVEL_STATIC;
	m_pChip = static_pointer_cast<CUi_Default>(CUi_Mgr::Get_Instance()
		->Add_Clone_Return(CUi_Default::ObjID, m_iLevelIndex, L"CUi", CUi_Default::ObjID, &Desc));
}
