	...
	:CUi(pDevice, pContext) 
{
	...
	XMStoreFloat4x4(&m_ProjMatrix,
		XMMatrixOrthographicLH((_float)(g_iWinSizeX >> 1), (_float)(g_iWinSizeY >> 1), 0.f, 1.f));
#else
	XMStoreFloat4x4(&m_ProjMatrix,
		XMMatrixOrthographicLH((_float)(g_iWinSizeX), (_float)(g_iWinSizeY), 0.f, 1.f));
#endif
...
void CUi_2D::Add_Pos(_float PosX, _float PosY)
{
	_float3 Pos = Get_Pos();
	Pos.x += PosX;
	Pos.y += PosY;
...
{
	FontDescInfo RenderFontDesc = Desc;
#ifdef PERFECTION_LOAD
	RenderFontDesc.Size *= 2;
#endif
	auto pGameInstance = m_pGameInstance.lock();
	if (EnterCount == 0)
	{
		if (RenderFontDesc.String.size() != 0)
		{
			pGameInstance->Render_Font(RenderFontDesc.FontTag, RenderFontDesc.String,
				Change_APICoordinate(RenderFontDesc.DirectPos), XMLoadFloat4(&RenderFontDesc.Color), RenderFontDesc.Rotation, RenderFontDesc.Size);
		}
	}
	else
	{
		...
		for (_uint i = 0; i < EnterCount; ++i)
		{
			...
			pGameInstance->Render_Font(TempFont.FontTag, TempFont.String,
...
_uint CUi_2D::Num_Quotation(FontDescInfo& Desc, _int Number)
{
	_int Digit = Number;
	_int SingleDigit = 0;
	_int Count = { 0 };
	...
	while (Count > 0)
	{
		...
		_float Pow = pow(10, Count - 1);
		SingleDigit = _int(Number / Pow);
		Desc.String += to_wstring(SingleDigit);
		Number -= _int(SingleDigit * Pow);
