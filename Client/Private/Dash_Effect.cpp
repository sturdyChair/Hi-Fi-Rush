...
wstring CDash_Effect::ObjID = TEXT("CDash_Effect");

CDash_Effect::CDash_Effect(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CEffect_Base{ pDevice, pContext }
{
	s_PrototypeTags.insert(CDash_Effect::ObjID);
}
...
HRESULT CDash_Effect::Initialize_Prototype()
{
	...
	m_List_ResourceInfo.push_back(make_shared<RES_TEXTURE>(TEXT("Prototype_Component_Effect_T_VFX_tk_swing_12"), TEXT("../Bin/Resources/Effect/Dash/T_VFX_tk_swing_12.png"), 1));
...
void CDash_Effect::EffectStartCallBack()
{
}

void CDash_Effect::EffectEndCallBack()
{

}

void CDash_Effect::EffectSoftEnd()
{
}

void CDash_Effect::EffectHardEnd()
{
}

HRESULT CDash_Effect::Ready_Components()
{
	/* For.Com_Shader */
	...
	m_pNoiseTextureCom = static_pointer_cast<CTexture>(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Effect_T_VFX_tk_scratch_swing_12"), TEXT("Com_Noise_Texture")));

	m_pTextureCom = static_pointer_cast<CTexture>(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Effect_T_VFX_tk_swing_12"), TEXT("Com_Texture")));
...
shared_ptr<CDash_Effect> CDash_Effect::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	struct MakeSharedEnabler :public CDash_Effect
	{
		MakeSharedEnabler(ID3D11Device* pDevice, ID3D11DeviceContext* pContext) : CDash_Effect(pDevice, pContext)
		{}
	};


	shared_ptr<CDash_Effect> pInstance = make_shared<MakeSharedEnabler>(pDevice, pContext);
...
shared_ptr<CGameObject> CDash_Effect::Clone(void* pArg)
{
	struct MakeSharedEnabler :public CDash_Effect
	{
		MakeSharedEnabler(const CDash_Effect& rhs) : CDash_Effect(rhs) {}
	};

	shared_ptr<CDash_Effect> pInstance = make_shared<MakeSharedEnabler>(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : CDash_Effect"));
		assert(false);
	}
