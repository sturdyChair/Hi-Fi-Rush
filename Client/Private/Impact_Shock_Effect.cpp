...
wstring CImpact_Shock_Effect::ObjID = TEXT("CImpact_Shock_Effect");

CImpact_Shock_Effect::CImpact_Shock_Effect(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CEffect_Base{ pDevice, pContext }
{
	s_PrototypeTags.insert(CImpact_Shock_Effect::ObjID);
}

CImpact_Shock_Effect::CImpact_Shock_Effect(const CImpact_Shock_Effect& rhs)
	: CEffect_Base(rhs)
{
}

CImpact_Shock_Effect::~CImpact_Shock_Effect()
{
	Free();
}

HRESULT CImpact_Shock_Effect::Initialize_Prototype()
{
...
HRESULT CImpact_Shock_Effect::Initialize(void* pArg)
{
...
void CImpact_Shock_Effect::PriorityTick(_float fTimeDelta)
{


}

void CImpact_Shock_Effect::Tick(_float fTimeDelta)
{

	EffectTick(fTimeDelta);
}

void CImpact_Shock_Effect::LateTick(_float fTimeDelta)
{
	if (GetEffectIsView()) {
...
HRESULT CImpact_Shock_Effect::Render()
{
...
void CImpact_Shock_Effect::EffectStartCallBack()
{
}

void CImpact_Shock_Effect::EffectEndCallBack()
{

}

void CImpact_Shock_Effect::EffectSoftEnd()
{
}

void CImpact_Shock_Effect::EffectHardEnd()
{
}

HRESULT CImpact_Shock_Effect::Ready_Components()
{
	/* For.Com_Shader */
...
shared_ptr<CImpact_Shock_Effect> CImpact_Shock_Effect::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	struct MakeSharedEnabler :public CImpact_Shock_Effect
	{
		MakeSharedEnabler(ID3D11Device* pDevice, ID3D11DeviceContext* pContext) : CImpact_Shock_Effect(pDevice, pContext)
		{}
	};


	shared_ptr<CImpact_Shock_Effect> pInstance = make_shared<MakeSharedEnabler>(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : CImpact_Shock_Effect"));
		assert(false);
	}
...
shared_ptr<CGameObject> CImpact_Shock_Effect::Clone(void* pArg)
{
	struct MakeSharedEnabler :public CImpact_Shock_Effect
	{
		MakeSharedEnabler(const CImpact_Shock_Effect& rhs) : CImpact_Shock_Effect(rhs) {}
	};

	shared_ptr<CImpact_Shock_Effect> pInstance = make_shared<MakeSharedEnabler>(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : CImpact_Shock_Effect"));
		assert(false);
	}
...
void CImpact_Shock_Effect::Free()
{
}