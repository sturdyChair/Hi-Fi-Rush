...
wstring CFont_Effect_Bam_M::ObjID = TEXT("CFont_Effect_Bam_M");

CFont_Effect_Bam_M::CFont_Effect_Bam_M(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CEffect_Base{ pDevice, pContext }
{
	s_PrototypeTags.insert(CFont_Effect_Bam_M::ObjID);
}

CFont_Effect_Bam_M::CFont_Effect_Bam_M(const CFont_Effect_Bam_M& rhs)
	: CEffect_Base(rhs)
{
}

CFont_Effect_Bam_M::~CFont_Effect_Bam_M()
{
	Free();
}

HRESULT CFont_Effect_Bam_M::Initialize_Prototype()
{
...
HRESULT CFont_Effect_Bam_M::Initialize(void* pArg)
{
...
void CFont_Effect_Bam_M::PriorityTick(_float fTimeDelta)
{


}

void CFont_Effect_Bam_M::Tick(_float fTimeDelta)
{

	EffectTick(fTimeDelta);
}

void CFont_Effect_Bam_M::LateTick(_float fTimeDelta)
{
	if (GetEffectIsView()) {
...
HRESULT CFont_Effect_Bam_M::Render()
{
...
void CFont_Effect_Bam_M::EffectStartCallBack()
{
}

void CFont_Effect_Bam_M::EffectEndCallBack()
{

}

void CFont_Effect_Bam_M::EffectSoftEnd()
{
}

void CFont_Effect_Bam_M::EffectHardEnd()
{
}

HRESULT CFont_Effect_Bam_M::Ready_Components()
{
	/* For.Com_Shader */
...
shared_ptr<CFont_Effect_Bam_M> CFont_Effect_Bam_M::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	struct MakeSharedEnabler :public CFont_Effect_Bam_M
	{
		MakeSharedEnabler(ID3D11Device* pDevice, ID3D11DeviceContext* pContext) : CFont_Effect_Bam_M(pDevice, pContext)
		{}
	};


	shared_ptr<CFont_Effect_Bam_M> pInstance = make_shared<MakeSharedEnabler>(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : CFont_Effect_Bam_M"));
		assert(false);
	}
...
shared_ptr<CGameObject> CFont_Effect_Bam_M::Clone(void* pArg)
{
	struct MakeSharedEnabler :public CFont_Effect_Bam_M
	{
		MakeSharedEnabler(const CFont_Effect_Bam_M& rhs) : CFont_Effect_Bam_M(rhs) {}
	};

	shared_ptr<CFont_Effect_Bam_M> pInstance = make_shared<MakeSharedEnabler>(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : CFont_Effect_Bam_M"));
		assert(false);
	}
