...
wstring CFont_Effect_HNGH::ObjID = TEXT("CFont_Effect_HNGH");

CFont_Effect_HNGH::CFont_Effect_HNGH(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CEffect_Base{ pDevice, pContext }
{
	s_PrototypeTags.insert(CFont_Effect_HNGH::ObjID);
}

CFont_Effect_HNGH::CFont_Effect_HNGH(const CFont_Effect_HNGH& rhs)
	: CEffect_Base(rhs)
{
}

CFont_Effect_HNGH::~CFont_Effect_HNGH()
{
	Free();
}

HRESULT CFont_Effect_HNGH::Initialize_Prototype()
{
	if (FAILED(__super::Initialize_Prototype()))
		return E_FAIL;

	m_List_ResourceInfo.push_back(make_shared<RES_TEXTURE>(TEXT("Prototype_Component_Effect_HNGH_Font"), TEXT("../Bin/Resources/Effect/Font/Korsica/T_VFX_tk_word_tc_HNGH_01.dds"), 1));
...
HRESULT CFont_Effect_HNGH::Initialize(void* pArg)
{
...
void CFont_Effect_HNGH::PriorityTick(_float fTimeDelta)
{


}

void CFont_Effect_HNGH::Tick(_float fTimeDelta)
{

	EffectTick(fTimeDelta);
}

void CFont_Effect_HNGH::LateTick(_float fTimeDelta)
{
	if (GetEffectIsView()) {
...
HRESULT CFont_Effect_HNGH::Render()
{
...
void CFont_Effect_HNGH::EffectStartCallBack()
{
}

void CFont_Effect_HNGH::EffectEndCallBack()
{

}

void CFont_Effect_HNGH::EffectSoftEnd()
{
}

void CFont_Effect_HNGH::EffectHardEnd()
{
}

HRESULT CFont_Effect_HNGH::Ready_Components()
{
	/* For.Com_Shader */
	...
	m_pTextureCom = static_pointer_cast<CTexture>(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Effect_HNGH_Font"), TEXT("Com_Texture")));
...
shared_ptr<CFont_Effect_HNGH> CFont_Effect_HNGH::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	struct MakeSharedEnabler :public CFont_Effect_HNGH
	{
		MakeSharedEnabler(ID3D11Device* pDevice, ID3D11DeviceContext* pContext) : CFont_Effect_HNGH(pDevice, pContext)
		{}
	};


	shared_ptr<CFont_Effect_HNGH> pInstance = make_shared<MakeSharedEnabler>(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : CFont_Effect_HNGH"));
		assert(false);
	}
...
shared_ptr<CGameObject> CFont_Effect_HNGH::Clone(void* pArg)
{
	struct MakeSharedEnabler :public CFont_Effect_HNGH
	{
		MakeSharedEnabler(const CFont_Effect_HNGH& rhs) : CFont_Effect_HNGH(rhs) {}
	};

	shared_ptr<CFont_Effect_HNGH> pInstance = make_shared<MakeSharedEnabler>(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : CFont_Effect_HNGH"));
		assert(false);
	}
