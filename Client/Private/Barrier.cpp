...
void CBarrier::LateTick(_float fTimeDelta)
{
	if (GetEffectIsView()) {
		...
		if(m_bOff)
			...
			m_pGameInstance.lock()->Add_RenderObjects(CRenderer::RENDER_BLEND_NON_LIT, shared_from_this());
		GAMEINSTANCE->Add_Collider(COLLIDER_GROUP::COLLISION_MONSTER, m_pCollider);
	}
}
...
HRESULT CBarrier::Render()
{
	...
	m_pShaderCom->Bind_Vector4("g_vCamPos", &pGameInstance->Get_CamPosition_Float4());
...
void CBarrier::TakeDamage(const DamageInfo& damageInfo)
{
	if (damageInfo.eAttackType == EAttackType::Peppermint)
	{
		m_pOwner.lock()->SetBarrierOff();
	}
}
