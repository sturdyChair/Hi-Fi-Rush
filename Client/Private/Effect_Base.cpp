...
void CEffect_Base::EffectParamBind(shared_ptr<CShader> _pShader)
{
	...
	_pShader->Bind_RawValue("g_fEffectColorBlendRate", &m_fEffectColorBlendRate, sizeof(_float));
...
void CEffect_Base::TimeAcc(_float fTimeDelta)
{
	...
	if(!GetEffectTimeFreeze()) SetEffectTimeAcc(GetEffectTimeAcc() + (fTimeDelta * GetEffectTimeAccMultiplier() * GetLinkTimeAccMultiplier()));
...
_float CEffect_Base::GetLinkTimeAccMultiplier()
{
	return m_pParentUnion.lock() ? m_pParentUnion.lock()->GetUnionTimeMultiplier() : 1.f;
}
