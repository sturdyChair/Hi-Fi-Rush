...
wstring CRekka_Spark_Lightning::ObjID = TEXT("CRekka_Spark_Lightning");

CRekka_Spark_Lightning::CRekka_Spark_Lightning(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CEffect_Base{ pDevice, pContext }
{
	s_PrototypeTags.insert(CRekka_Spark_Lightning::ObjID);
}

CRekka_Spark_Lightning::CRekka_Spark_Lightning(const CRekka_Spark_Lightning& rhs)
	: CEffect_Base(rhs)
{
}

CRekka_Spark_Lightning::~CRekka_Spark_Lightning()
{
	Free();
}

HRESULT CRekka_Spark_Lightning::Initialize_Prototype()
{
...
HRESULT CRekka_Spark_Lightning::Initialize(void* pArg)
{
...
void CRekka_Spark_Lightning::PriorityTick(_float fTimeDelta)
{


}

void CRekka_Spark_Lightning::Tick(_float fTimeDelta)
{
...
void CRekka_Spark_Lightning::LateTick(_float fTimeDelta)
{
	if (GetEffectIsView()) {
...
HRESULT CRekka_Spark_Lightning::Render()
{
	_float4x4 ViewMat;
	...
	{
		_float4x4 Temp = GAMEINSTANCE->Matrix_BillBoard(m_pTransformCom->Get_WorldMatrix());
...
void CRekka_Spark_Lightning::EffectStartCallBack()
{
...
void CRekka_Spark_Lightning::EffectEndCallBack()
{

}

void CRekka_Spark_Lightning::EffectSoftEnd()
{
}

void CRekka_Spark_Lightning::EffectHardEnd()
{
}

HRESULT CRekka_Spark_Lightning::Ready_Components()
{
	/* For.Com_Shader */
...
shared_ptr<CRekka_Spark_Lightning> CRekka_Spark_Lightning::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	struct MakeSharedEnabler :public CRekka_Spark_Lightning
	{
		MakeSharedEnabler(ID3D11Device* pDevice, ID3D11DeviceContext* pContext) : CRekka_Spark_Lightning(pDevice, pContext)
		{}
	};


	shared_ptr<CRekka_Spark_Lightning> pInstance = make_shared<MakeSharedEnabler>(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : CRekka_Spark_Lightning"));
		assert(false);
	}
...
shared_ptr<CGameObject> CRekka_Spark_Lightning::Clone(void* pArg)
{
	struct MakeSharedEnabler :public CRekka_Spark_Lightning
	{
		MakeSharedEnabler(const CRekka_Spark_Lightning& rhs) : CRekka_Spark_Lightning(rhs) {}
	};

	shared_ptr<CRekka_Spark_Lightning> pInstance = make_shared<MakeSharedEnabler>(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : CRekka_Spark_Lightning"));
		assert(false);
	}
...
void CRekka_Spark_Lightning::Free()
{
}