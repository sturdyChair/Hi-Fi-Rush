...
public:
	...
	enum EFFECT_TYPE { EFFECT_NONE, EFFECT_MESH, EFFECT_PARTICLE, EFFECT_TEXTURE, EFFECT_TEXTURE_BILLBOARD, EFFECT_DAMAGE };
...
public:
	...
	_float GetLinkTimeAccMultiplier();
	...
	_bool Get_LoopObject() { return m_bEffectLoopObject; }
	void Set_LoopObject(_bool _bLoopObject) { m_bEffectLoopObject = _bLoopObject; }

	void Set_ParentWorld(_float4x4* _pParentWorld) { m_pParentWorld = _pParentWorld; }

	void Set_ParentUnion(shared_ptr<class CEffect_Union> _pParentUnion) { m_pParentUnion = _pParentUnion; }
protected:
	...
	_bool		m_bEffectLoopObject = false;
	...
	_float4x4  m_matRoot = {};
	_float4x4* m_pParentWorld = nullptr;

	EFFECT_TYPE m_eEffectType = EFFECT_TYPE::EFFECT_NONE;

	weak_ptr<class CEffect_Union> m_pParentUnion;
